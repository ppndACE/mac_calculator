...
- (void) On_RegOp:(NSString *)op
{
    ...
    /* add previous number to the numbers array */
    //[eq addObject:[NSString stringWithFormat:@"%f", current_value]];
...
- (IBAction) On_Equals:(id)sender
{
    ...
    if (!equals_was_last_called) {
        ...
        NSNumber *result = [e performShuntingYardComputationWithEquation:eq];
        
        [self WriteToAnswerBox:[result stringValue]];
    }
...
- (NSString *) WriteToAnswerBox:(NSString *)i
{
    ...
    if ([answer_box intValue] == 0) { /* if value is 0, replace it, otherwise append */
        [answer_box setStringValue:i];
    }
    else { /* includes case where decimal has been placed */
